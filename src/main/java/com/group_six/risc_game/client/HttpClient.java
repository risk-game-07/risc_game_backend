...
public class HttpClient{
    ...
    public String sendAttack(String playerId, String roomId, GameActionReq gameActionReq){
        String url = "http://localhost:8080/game/start/sendGameAction";
        JSONObject json = new JSONObject();
        json.put("playerId", playerId);
        json.put("roomId", roomId);
        ...
        /*
            ...
            int curPhase;
         */
        return getString(result, "data", "errMess");
    }

    public String sendMove(String playerId, String roomId, GameActionReq gameActionReq){
        String url = "http://localhost:8080/game/start/sendGameAction";
        JSONObject json = new JSONObject();
        json.put("playerId", playerId);
        json.put("roomId", roomId);
    ...
    public boolean moveNextPhase(int curPhase, String playerId, String roomId){
        ...
        RestTemplate restTemplate = new RestTemplate();
        String result = restTemplate.postForObject(url, json, String.class);
        boolean ans = getBoolean(result, "data", "end");
        if(ans)
            isEnd = getInt(result, "data", "isEndOfGame");
        return ans;
    }
